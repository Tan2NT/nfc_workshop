...
import com.tanhoang.cardreader.model.Applet
...
object TlvUtil {
    ...
    fun getApplicationList(data: ByteArray?): List<Applet> {
        var data = data
        val appList: MutableList<Applet> = ArrayList()
        ...
        for (app in getMultipleTlv(data, TlvTagConstant.APP_TLV_TAG)) {
            ...
            if (appPriorityInd != null) {
                ...
                if (appPriorityIndStr != null) priority = appPriorityIndStr.toInt()
            }
            val application = Applet(
                aid = aid,
                appLabel = if(appLabel != null) HexUtil.bytesToAscii(appLabel) else null,
                priority = priority
            )
